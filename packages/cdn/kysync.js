...
function DepositsKysync({
    ...
    if (document.getElementById("deposits_checkout_frame_wrapper")) {
    } else {
        if (location.host.search('deposits.com') > -1 || location.host.search('deposits.inc') > -1) {
            live = true
        }
        let apiUrl = live ?
        ...
        if (entity_type != 'business' && entity_type != 'user') {
            console.log('Entity type must be user or business')
        } else {
            const generateUrl = `${apiUrl}/${public_key}/${entity_type}/${access_id}`;

            // console.log(generateUrl);
            iframe = document.createElement("iframe");
            iframe.src = generateUrl;
            iframe.allow = "camera 'self' https://*.deposits.com https://*.deposits.dev http://localhost:3000"
            ...
            wrapper.id = "deposits_checkout_frame_wrapper";
